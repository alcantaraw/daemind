        ...
        for container in $containers; do
            ...
            case "$servico" in
                ...
                postiz)
                    ...
                    ;;
                minio)
                    versao=$(docker exec "$container" minio --version 2>/dev/null | grep -o 'RELEASE\.[0-9T-]*' || echo "")
                    ;;
                temporal|pooler|*)
...
# ===============================================================================
    mkdir -p "$TARGET_DIR"/volumes/{postgres_data,n8n_data,evolution_instances,nocodb_data,tailscale_state,nocodb_ts_state,caddy_data,minio_data,litellm_data,openwebui_data}
    mkdir -p "$TARGET_DIR"/pgbouncer_conf
    mkdir -p "$TARGET_DIR"/volumes/minio_data/postiz
    ...
    chown -R 1000:1000 "$TARGET_DIR/volumes/n8n_data" "$TARGET_DIR/volumes/evolution_instances" "$TARGET_DIR/volumes/nocodb_data" "$TARGET_DIR/volumes/minio_data" "$TARGET_DIR/volumes/openwebui_data" "$TARGET_DIR/volumes/litellm_data" 2>/dev/null || true
...
# =====================================================================================
cp "$RAIZ_REPO/core/config/docker-compose.yml" ./docker-compose.yml
...
# [SRE DOC] Suporte Dinâmico NocoDB: Substitui a porta hardcoded 8080 pela variável de ambiente do projeto
# sed -i 's/"8080:8080\/tcp"/"${NOCO_PORT:-18080}:8080\/tcp"/g' ./docker-compose.yml
...
echo "➜ [SRE] Verificando e inicializando infraestrutura de dados (Postgres, Redis, PgBouncer & MinIO)..."
INFRA_SERVICES=("postgres" "redis" "pgbouncer" "minio")
INFRA_OFFLINE=()
...
if [ ${#INFRA_OFFLINE[@]} -eq 0 ]; then
    echo "➜ [IDEMPOTÊNCIA] Todos os serviços de dados/cache (Postgres, Redis, PgBouncer, MinIO) já estão ativos em status RUNNING."
else
    echo "  ↳ Disparando subida seletiva dos serviços offline: ${INFRA_OFFLINE[*]}..."
    if ! DB_UP_ERR=$(docker compose up -d "${INFRA_OFFLINE[@]}" 2>&1); then
...
echo "1. Aguardando prontidão do banco Postgres e MinIO..."
TENTATIVAS_DB=0
...
TENTATIVAS_MINIO=0
until [ "$(docker inspect -f '{{.State.Health.Status}}' ${PREFIXO_CONTAINER}_minio 2>/dev/null)" = "healthy" ]; do
  TENTATIVAS_MINIO=$((TENTATIVAS_MINIO+1))
  [ "$TENTATIVAS_MINIO" -ge 30 ] && { echo "🚨 [ERRO FATAL] MinIO não atingiu estado saudável após 60s."; exit 1; }
  sleep 2
done

echo "➜ [SRE] Verificando e garantindo buckets padrões no MinIO S3 (Idempotente)..."
docker exec ${PREFIXO_CONTAINER}_minio mc alias set local http://localhost:9000 ${TS_EMAIL} ${DB_PASSWORD} >/dev/null 2>&1 || true
docker exec ${PREFIXO_CONTAINER}_minio mc ls local/chatwoot >/dev/null 2>&1 || docker exec ${PREFIXO_CONTAINER}_minio mc mb local/chatwoot >/dev/null 2>&1 || true
docker exec ${PREFIXO_CONTAINER}_minio mc anonymous set download local/chatwoot >/dev/null 2>&1 || true
docker exec ${PREFIXO_CONTAINER}_minio mc ls local/postiz >/dev/null 2>&1 || docker exec ${PREFIXO_CONTAINER}_minio mc mb local/postiz >/dev/null 2>&1 || true
docker exec ${PREFIXO_CONTAINER}_minio mc anonymous set download local/postiz >/dev/null 2>&1 || true
}
...
if ! docker compose exec -T postgres psql -U $DB_USER -d chatwoot_db -c "SELECT 1 FROM installation_configs LIMIT 1;" > /dev/null 2>&1 < /dev/null; then
    ...
    docker compose run --rm -T chatwoot bundle exec rails db:chatwoot_prepare > /dev/null 2>&1 < /dev/null || true
else
...
if [ ${#APPS_OFFLINE[@]} -eq 0 ]; then
    ...
    echo "➜ [IDEMPOTÊNCIA & SRE] ${SERVICOS_RODANDO} de ${#ALL_DECLARED_SERVICES[@]} serviços declarados no docker-compose.yml operando em status RUNNING."
fi
...
if [ "$HEALTH_MINIO" = "healthy" ]; then
    HTTP_MINIO_TS=$(curl -s -L -H "Host: ${TS_DOMAIN}" -o /dev/null -w "%{http_code}" --max-time 10 --retry 3 --retry-delay 2 "http://127.0.0.1:9001/" || echo "FALHOU")
else
    HTTP_MINIO_TS="CONTAINER_ERRO"
fi
...
echo "  ↳ Acesso MinIO Console:   http://$TS_DOMAIN:9001  -> Status: [$HTTP_MINIO_TS]"
