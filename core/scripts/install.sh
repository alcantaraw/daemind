...
CLIENT_ENV_FILE="${1:-${RAIZ_REPO}/core/config/.env}"

# Se o .env oficial não for indicado, tenta qualquer arquivo .env em core/config/
if [ ! -f "$CLIENT_ENV_FILE" ]; then
    CLIENT_ENV_FILE=$(ls -t "${RAIZ_REPO}/core/config/"*.env 2>/dev/null | head -n 1 || true)
fi
...
if [ -n "$CLIENT_ENV_FILE" ] && [ -f "$CLIENT_ENV_FILE" ]; then
    ...
    # Consolidação Atômica SSOT: Garante core/config/.env como a única Fonte da Verdade
    TARGET_SSOT="${RAIZ_REPO}/core/config/.env"
    ...
    echo "====================================================================="
    echo "🚨 [ERRO FATAL] Arquivo de configuração (.env) não encontrado em ${RAIZ_REPO}/core/config/.env!"
...
run_step() {
    ...
    if [ "$FORCE_MODE" = "false" ] && grep -q "^${STEP_ID}$" "$STATE_FILE" 2>/dev/null; then
        ...
        # Dispara a função isolada
        $CMD_FUNC

        # Grava no state file garantindo a continuidade em caso de crash
...
# ===============================================================================
    mkdir -p "$TARGET_DIR"/volumes/{postgres_data,n8n_data,evolution_instances,nocodb_data,tailscale_state,nocodb_ts_state,caddy_data,litellm_data,openwebui_data,postiz_data}
    mkdir -p "$TARGET_DIR"/pgbouncer_conf
    ...
    chown -R 1000:1000 "$TARGET_DIR/volumes/n8n_data" "$TARGET_DIR/volumes/evolution_instances" "$TARGET_DIR/volumes/nocodb_data" "$TARGET_DIR/volumes/openwebui_data" "$TARGET_DIR/volumes/litellm_data" "$TARGET_DIR/volumes/postiz_data" 2>/dev/null || true
...
# 2. SRE Guardrail: Ajusta permissão apenas dos arquivos de código/config (preservando volumes)
    if [ -n "$SUDO_USER" ]; then
        ...
        chown -R "$SUDO_USER:$SUDO_USER" "$TARGET_DIR/core" "$TARGET_DIR/pgbouncer_conf" 2>/dev/null || true
    fi
...
# -------------------------------------------------------------------------
	...
	export TS_DOMAIN
	if [ -n "$TS_DOMAIN" ]; then
	    if grep -q '^TS_DOMAIN=' "${TARGET_DIR}/core/config/.env" 2>/dev/null; then
	        sudo sed -i "s|^TS_DOMAIN=.*|TS_DOMAIN=\"${TS_DOMAIN}\"|" "${TARGET_DIR}/core/config/.env" 2>/dev/null || true
	    fi
	fi
}
...
SSOT_PATH="${TARGET_DIR}/core/config/.env"
[ ! -f "$SSOT_PATH" ] && SSOT_PATH="${RAIZ_REPO}/core/config/.env"
...
chmod 600 .env
mkdir -p "$TARGET_DIR/core/config" 2>/dev/null || true
cp .env "$TARGET_DIR/core/config/.env" 2>/dev/null || true
chmod 600 "$TARGET_DIR/core/config/.env" 2>/dev/null || true
}
...
# =====================================================================================
cd "$TARGET_DIR" 2>/dev/null || true
COMPOSE_FILES=("--env-file" "./core/config/.env" "-p" "${PREFIXO_CONTAINER:-daemind}" "-f" "./core/config/docker-compose.yml")
if [[ "${USE_MINIO:-s}" =~ ^[Ss]$ ]] && [ -f "./core/config/docker-compose.minio.yml" ]; then
    COMPOSE_FILES+=("-f" "./core/config/docker-compose.minio.yml")
fi
...
if [ "$USE_TAILSCALE" = "false" ]; then
    ...
    sed -i -E '/\$\{EVO_PORT.*:8081\/tcp/d' ./core/config/docker-compose.yml
    
    if [ "$CADDY_PROTOCOL" = "https" ]; then
        sed -i 's/- "${PROXY_PORT:-80}:80\/tcp"/- "80:80\/tcp"\n      - "443:443\/tcp"/g' ./core/config/docker-compose.yml
    fi
fi
...
while [ $TENTATIVAS_PULL -le $MAX_TENTATIVAS_PULL ]; do
    ...
    if PULL_ERRORS=$(docker compose "${COMPOSE_FILES[@]}" pull --quiet 2>&1 >/dev/null); then
        SUCESSO_PULL=true
        break
    fi
...
TOTAL_SERVICOS=$(docker compose "${COMPOSE_FILES[@]}" config --services 2>/dev/null | grep -v '^$' | wc -l | xargs 2>/dev/null || echo "13")
IMAGENS_PULLED=$(docker compose "${COMPOSE_FILES[@]}" images --quiet 2>/dev/null | grep -v '^$' | sort -u | wc -l | xargs 2>/dev/null || echo "$TOTAL_SERVICOS")
...
for svc in "${INFRA_SERVICES[@]}"; do
    STATUS=$(docker compose "${COMPOSE_FILES[@]}" ps --services --filter "status=running" 2>/dev/null | grep -w "$svc" || echo "")
    if [ -z "$STATUS" ]; then
        INFRA_OFFLINE+=("$svc")
    fi
done
...
if [ ${#INFRA_OFFLINE[@]} -eq 0 ]; then
    ...
    if ! DB_UP_ERR=$(docker compose "${COMPOSE_FILES[@]}" up -d "${INFRA_OFFLINE[@]}" 2>&1); then
...
echo "1. Aguardando prontidão do banco Postgres..."
TENTATIVAS_DB=0
until docker compose "${COMPOSE_FILES[@]}" exec -T postgres pg_isready -U $DB_USER -d ${PREFIXO_CONTAINER}_db > /dev/null 2>&1 < /dev/null; do
  TENTATIVAS_DB=$((TENTATIVAS_DB+1))
  [ "$TENTATIVAS_DB" -ge 30 ] && { echo "🚨 [ERRO FATAL] Postgres não respondeu após 60s."; exit 1; }
  sleep 2
done

# --- INVOCAÇÃO PONTUAL DAS ATIVIDADES ESPECÍFICAS DO MINIO S3 ---
if [[ "${USE_MINIO:-s}" =~ ^[Ss]$ ]]; then
    ...
    echo "➜ [SRE SKIP] Módulo MinIO desativado (Utilizando Armazenamento S3 Externo)."
fi
}
...
docker compose "${COMPOSE_FILES[@]}" exec -T postgres psql -U $DB_USER -d ${PREFIXO_CONTAINER}_db -q < core/database/init.sql > /dev/null 2>&1 || true

DBS_CRIADOS=0
for db in evolution_db chatwoot_db postiz_db temporal temporal_visibility litellm_db openwebui_db; do
    if ! docker compose "${COMPOSE_FILES[@]}" exec -T postgres psql -U $DB_USER -d ${PREFIXO_CONTAINER}_db -c "SELECT 1 FROM pg_database WHERE datname = '$db'" < /dev/null 2>/dev/null | grep -q 1; then
        DBS_CRIADOS=$((DBS_CRIADOS+1))
        docker compose "${COMPOSE_FILES[@]}" exec -T postgres psql -U $DB_USER -d ${PREFIXO_CONTAINER}_db -q -c "CREATE DATABASE $db;" < /dev/null > /dev/null 2>&1 || true
    fi
done
...
if ! docker compose "${COMPOSE_FILES[@]}" exec -T postgres psql -U $DB_USER -d chatwoot_db -c "SELECT 1 FROM installation_configs LIMIT 1;" > /dev/null 2>&1 < /dev/null; then
    echo "➜ [CONFIGURANDO] Executando preparação de schema inicial do Chatwoot (db:chatwoot_prepare)..."
    docker compose "${COMPOSE_FILES[@]}" run --rm -T chatwoot bundle exec rails db:chatwoot_prepare > /dev/null 2>&1 < /dev/null || true
else
    echo "➜ [IDEMPOTÊNCIA] Banco do Chatwoot já estruturado. Executando migrações incrementais (db:migrate)..."
    docker compose "${COMPOSE_FILES[@]}" run --rm -T chatwoot bundle exec rails db:migrate > /dev/null 2>&1 < /dev/null || true
fi
...
ALL_DECLARED_SERVICES=($(docker compose "${COMPOSE_FILES[@]}" config --services 2>/dev/null | grep -v '^$' || true))
RUNNING_SERVICES=($(docker compose "${COMPOSE_FILES[@]}" ps --services --filter "status=running" 2>/dev/null | grep -v '^$' || true))
APPS_OFFLINE=()
...
if [ ${#APPS_OFFLINE[@]} -eq 0 ]; then
    ...
    if ! UP_ERR=$(docker compose "${COMPOSE_FILES[@]}" up -d --remove-orphans "${APPS_OFFLINE[@]}" 2>&1); then
        echo "🚨 [ERRO CRÍTICO] Falha ao injetar contêineres offline (${APPS_OFFLINE[*]}) no daemon do Docker!"
        echo "$UP_ERR"
        exit 1
    fi
    SERVICOS_RODANDO=$(docker compose "${COMPOSE_FILES[@]}" ps --services --filter "status=running" 2>/dev/null | grep -v '^$' | wc -l | xargs 2>/dev/null || echo "0")
...
# 4. Força o reinício do PM2 no Postiz para conectar ao Temporal sanitizado
echo "➜ [CONFIGURANDO] Inicializando barramento do Postiz com o Temporal..."
docker exec -i ${PREFIXO_CONTAINER}_postiz pm2 restart all > /dev/null 2>&1 || docker restart ${PREFIXO_CONTAINER}_postiz > /dev/null 2>&1 || true
...
if [[ "$RESPONSE_N8N" =~ ^2 ]]; then
    ...
    echo "➜ [AVISO] Setup n8n respondeu HTTP ${RESPONSE_N8N} (Ignorado com segurança)."
fi
...
if [[ "$RESPONSE_POSTIZ" =~ ^2 ]]; then
    ...
    echo "➜ [INFO] Setup do Postiz finalizado (HTTP ${RESPONSE_POSTIZ})."
fi
}
...
[ ! -f "$ENV_FILE" ] && ENV_FILE="${RAIZ_REPO}/core/config/.env"
...
HEALTH_DB=$(docker inspect -f '{{if .State.Health}}{{.State.Health.Status}}{{else}}unhealthy{{end}}' ${PREFIXO_CONTAINER}_db 2>/dev/null || echo "OFFLINE")
if [ "$HEALTH_DB" = "healthy" ]; then
    STATUS_POSTGRES=$(docker exec ${PREFIXO_CONTAINER}_db pg_isready -U "$DB_USER" -d "${PREFIXO_CONTAINER}_db" >/dev/null 2>&1 && echo " Saudável" || echo " FALHOU")
    STATUS_EVO_DB=$(docker exec ${PREFIXO_CONTAINER}_db pg_isready -U "$DB_USER" -d "evolution_db" >/dev/null 2>&1 && echo " Saudável" || echo " FALHOU")
else
...
HEALTH_POOLER=$(docker inspect -f '{{if .State.Health}}{{.State.Health.Status}}{{else}}unhealthy{{end}}' ${PREFIXO_CONTAINER}_pooler 2>/dev/null || echo "OFFLINE")
if [ "$HEALTH_POOLER" = "healthy" ] && [ "$HEALTH_DB" = "healthy" ]; then
    STATUS_BOUNCER=$(docker exec ${PREFIXO_CONTAINER}_db pg_isready -h ${PREFIXO_CONTAINER}_pooler -p 6432 >/dev/null 2>&1 && echo " Saudável" || echo " FALHOU")
else
...
# SRE CONSOLIDATED COMPOSE: Consolidação final do docker-compose.rendered.yml para consulta e auditoria
echo "➜ [SRE] Gerando snapshot auditável consolidado (docker-compose.rendered.yml)..."
COMPOSE_MERGE=("-f" "${TARGET_DIR}/core/config/docker-compose.yml")
if [[ "${USE_MINIO:-s}" =~ ^[Ss]$ ]] && [ -f "${TARGET_DIR}/core/config/docker-compose.minio.yml" ]; then
    COMPOSE_MERGE+=("-f" "${TARGET_DIR}/core/config/docker-compose.minio.yml")
fi
docker compose "${COMPOSE_MERGE[@]}" config > "${TARGET_DIR}/core/config/docker-compose.rendered.yml" 2>/dev/null || true
