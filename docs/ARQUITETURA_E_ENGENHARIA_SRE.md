...
A infraestrutura implementa a especificação de **Armazenamento Desacoplado Plugável (BYOS - Bring Your Own Storage)** com profilamento automático de hardware via motor `autotune.sh`:
- **Modo 1 - Armazenamento Local Direto (Custo/Recurso Otimizado):** Recomendado automaticamente para hosts modestos (< 4 Cores vCPU ou < 8GB RAM). Redireciona o armazenamento de anexos do Chatwoot e Postiz para volumes isolados no disco do servidor (`./volumes/chatwoot_data` e `./volumes/postiz_data`), reduzindo o consumo de memória RAM em ~1 GB ao desativar o contêiner do MinIO.
- **Modo 2 - MinIO S3 Soberano (`${PREFIXO_CONTAINER}_minio`):** Ativado em servidores de alta performance (>= 4 Cores e >= 8GB RAM). Executa o contêiner isolado do MinIO via *Compose Override* (`docker-compose.minio.yml`) nas portas `9000` (S3 API) e `9001` (Console UI), fornecendo compressão de disco (`MINIO_COMPRESS=on`) e gerenciamento centralizado de buckets.
- **Modo 3 - Provedor S3 Cloud Remoto (BYOS):** Conecta a stack a provedores externos de S3 (AWS S3, Cloudflare R2, DigitalOcean Spaces) injetando as credenciais e endpoints remotos diretamente no barramento de variáveis do Chatwoot e Postiz.
...
### 📦 4.9. Matriz Dinâmica de Versões e Imagens Docker (SRE BOM)
...
| :--- | :--- | :--- | :--- | :--- |
| `${PREFIXO_CONTAINER}_chatwoot` | `chatwoot/chatwoot` | `v4.16.0` | **4.16.0** | Inbox Omnichannel & Atendimento |
| `${PREFIXO_CONTAINER}_db` | `pgvector/pgvector` | `pg16` | **16.14** | Banco de Dados Relacional & Vetorial |
| `${PREFIXO_CONTAINER}_evolution` | `evoapicloud/evolution-api` | `v2.3.7` | **2.3.7** | API de Conexão WhatsApp |
| `${PREFIXO_CONTAINER}_litellm` | `ghcr.io/berriai/litellm` | `main-latest` *(dinâmica)* | **1.96.0** | Gateway & Roteador de Modelos de IA |
| `${PREFIXO_CONTAINER}_minio` | `minio/minio` | `latest` *(dinâmica)* | **2025-09-07T16-13-09** | Armazenamento S3 Soberano *(Módulo Opcional Desacoplado)* |
| `${PREFIXO_CONTAINER}_n8n` | `n8nio/n8n` | `2.31.6` | **2.31.6** | Motor de Automação de Processos |
| `${PREFIXO_CONTAINER}_nocodb` | `nocodb/nocodb` | `2026.07.0` | **2026.07.0** | CRM e Planilhas Inteligentes |
| `${PREFIXO_CONTAINER}_openwebui` | `ghcr.io/open-webui/open-webui` | `main` *(dinâmica)* | **0.11.0** | Interface Gráfica de IA (Chatbot) |
| `${PREFIXO_CONTAINER}_pooler` | `edoburu/pgbouncer` | `v1.25.2-p0` | **1.25.2-p0** | Multiplexador de Conexões Postgres |
| `${PREFIXO_CONTAINER}_postiz` | `ghcr.io/gitroomhq/postiz-app` | `v2.21.10` | **2.21.10** | Agendador de Redes Sociais |
| `${PREFIXO_CONTAINER}_redis` | `redis` | `7.4-alpine` | **7.4.10** | Cache & Fila de Automações em Memória |
| `${PREFIXO_CONTAINER}_temporal` | `temporalio/auto-setup` | `1.29.7` | **1.29.7** | Orquestrador de Workflows (Postiz) |
| `${PREFIXO_CONTAINER}_waf` | `caddy` | `2.11.4-alpine` | **2.11.4** | Reverse Proxy & WAF com SSL Automático |
