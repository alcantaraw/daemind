...
| **Armazenamento de Arquivos e Mídia** | AWS S3, Google Drive, Dropbox | R$ 150 – R$ 500/mês | **Incluso** *(MinIO S3 Soberano)* |
...
## 📦 Matriz de Versões da Stack & Imagens Docker (SRE BOM)

A infraestrutura do **daemind.** opera com a matriz de imagens e versões auditadas em tempo de execução pelo nosso pipeline de SRE (**SRE BOM - Bill of Materials**). 

Serviços configurados no `docker-compose.yml` com tags dinâmicas como `latest`, `main` ou `main-latest` possuem suas versões internas extraídas e fixadas de forma transparente pelo inspetor de runtime do sistema.

| Container | Imagem Docker | Tag no Compose | Versão Interna Auditada | Função na Stack |
| :--- | :--- | :--- | :--- | :--- |
| `loja_chatwoot` | `chatwoot/chatwoot` | `v4.16.0` | **4.16.0** | Inbox Omnichannel & Atendimento |
| `loja_db` | `pgvector/pgvector` | `pg16` | **16.14** | Banco de Dados Relacional & Vetorial |
| `loja_evolution` | `evoapicloud/evolution-api` | `v2.3.7` | **2.3.7** | API de Conexão WhatsApp |
| `loja_litellm` | `ghcr.io/berriai/litellm` | `main-latest` *(dinâmica)* | **1.96.0** | Gateway & Roteador de Modelos de IA |
| `loja_minio` | `minio/minio` | `latest` *(dinâmica)* | **2025-09-07T16-13-09** | Armazenamento S3 Soberano |
| `loja_n8n` | `n8nio/n8n` | `2.31.6` | **2.31.6** | Motor de Automação de Processos |
| `loja_nocodb` | `nocodb/nocodb` | `2026.07.0` | **2026.07.0** | CRM e Planilhas Inteligentes |
| `loja_openwebui` | `ghcr.io/open-webui/open-webui` | `main` *(dinâmica)* | **0.11.0** | Interface Gráfica de IA (Chatbot) |
| `loja_pooler` | `edoburu/pgbouncer` | `v1.25.2-p0` | **1.25.2-p0** | Multiplexador de Conexões Postgres |
| `loja_postiz` | `ghcr.io/gitroomhq/postiz-app` | `v2.21.10` | **2.21.10** | Agendador de Redes Sociais |
| `loja_redis` | `redis` | `7.4-alpine` | **7.4.10** | Cache & Fila de Automações em Memória |
| `loja_temporal` | `temporalio/auto-setup` | `1.29.7` | **1.29.7** | Orquestrador de Workflows (Postiz) |
| `loja_waf` | `caddy` | `2.11.4-alpine` | **2.11.4** | Reverse Proxy & WAF com SSL Automático |
